<template>
  <div class="space-y-6">
    ...
    <div class="flex items-start justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Quản lý Banner</h1>
        <p class="text-sm text-slate-500 mt-1">Xem, tạo mới và quản lý các banner quảng cáo trên hệ thống.</p>
      </div>
      <button
        @click="openCreate"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      <div class="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4 shadow-sm">
        ...
        <div>
          <div class="text-xs text-slate-500 uppercase tracking-wide font-semibold">Tổng số banner</div>
          <div class="text-2xl font-bold text-slate-900">{{ banners.length }}</div>
        </div>
      </div>
      <div class="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4 shadow-sm">
        ...
        <div>
          <div class="text-xs text-slate-500 uppercase tracking-wide font-semibold">Đang hiển thị</div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4 shadow-sm">
        ...
        <div>
          <div class="text-xs text-slate-500 uppercase tracking-wide font-semibold">Bản nháp</div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <div class="flex items-center gap-3">
          <h2 class="font-semibold text-slate-900">Danh sách banner</h2>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="text-slate-500 text-xs uppercase tracking-wide bg-slate-50 border-b border-slate-100">
              ...
              <th class="text-left px-5 py-3 font-medium">Ảnh banner</th>
              <th class="text-left px-5 py-3 font-medium">Tiêu đề &amp; Slug Link</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-5 py-3">
                ...
                <div class="text-xs text-slate-400">/banner/{{ item.slug }}</div>
              </td>
              <td class="px-5 py-3">
      ...
      <div class="flex items-center justify-between px-5 py-3 border-t border-slate-100 text-sm text-slate-500">
        <span>Hiển thị 1 - {{ filteredBanners.length }} của {{ banners.length }} banner</span>
          ...
          <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" v-if="showDetail && selected">
            ...
            <div class="relative">
              ...
              <span class="absolute top-3 right-3 text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1.5 bg-white border border-slate-200" :class="statusStyle(selected.status)">
                ...
                {{ selected.status === 'Active' ? 'Đang hiển thị' : 'Bản nháp' }}
              </span>
            </div>
            ...
            <div class="px-6 py-5">
              ...
              <p class="text-sm text-slate-700 leading-relaxed bg-slate-50 p-4 border border-slate-100 rounded-xl">{{ selected.description }}</p>
            </div>
    ...
    <!-- ============================================================ -->
    <!-- MODAL 2: THÊM MỚI / CHỈNH SỬA BANNER                           -->
    <!-- ============================================================ -->
    <Transition name="fade">
          ...
          <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" v-if="showForm && form">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
              <h2 class="font-semibold text-slate-900 flex items-center gap-2">
                ...
                {{ isCreating ? 'Thêm banner mới' : 'Chỉnh sửa banner' }}
              </h2>
              <button @click="closeAll" class="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            ...
            <div class="px-6 py-5 space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="text-sm font-medium text-slate-700">Tên banner <span class="text-rose-500">*</span></label>
                ...
                <div>
                  <label class="text-sm font-medium text-slate-700">Đường dẫn liên kết (Slug) <span class="text-rose-500">*</span></label>
              ...
              <div>
                <label class="text-sm font-medium text-slate-700">Mô tả banner / Ghi chú</label>
              ...
              <div>
                <label class="text-sm font-medium text-slate-700 block mb-2">Hình ảnh banner</label>

                <div v-if="form.image" class="flex items-start gap-4">
                  <img :src="form.image" class="w-48 h-24 object-cover rounded-lg border border-slate-200" />
                  <div class="text-sm">
                    <button @click="form.image = ''" class="text-rose-600 font-semibold hover:underline">Xóa ảnh</button>
                    <p class="text-xs text-slate-400 mt-2">Định dạng hỗ trợ: JPG, PNG, WEBP. Kích thước tối đa: 3MB.</p>
                  </div>
                </div>
                <div v-else class="flex items-start gap-4">
                  <div class="w-48 h-24 rounded-lg bg-slate-100 border border-dashed border-slate-200"></div>
                  <div class="text-sm">
            ...
            <div class="flex justify-end gap-2 px-6 py-4 border-t border-slate-100 bg-white">
              ...
              <button @click="submitForm" class="inline-flex items-center gap-1 px-4 py-2 text-sm rounded-lg bg-[#0258cb] text-white hover:bg-[#004bb3]">
                💾 {{ isCreating ? 'Tạo banner' : 'Cập nhật banner' }}
              </button>
            </div>
          </div>
        </Transition>
      ...
      <div v-if="showDeleteConfirm && toDelete" class="fixed inset-0 bg-slate-900/50 backdrop-blur-[2px] flex items-center justify-center p-4 z-50" @click.self="showDeleteConfirm = false">
        <Transition name="pop" appear>
          <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6" v-if="showDeleteConfirm && toDelete">
            <h3 class="font-semibold text-slate-900 mb-2">Xoá banner?</h3>
            <p class="text-sm text-slate-500 mb-5">
              Bạn có chắc muốn xoá banner "<span class="font-medium text-slate-700">{{ toDelete.title }}</span>"? Hành động này không thể hoàn tác.
            </p>
            <div class="flex justify-end gap-2">
              ...
              <button @click="confirmDelete" class="px-4 py-2 text-sm rounded-lg bg-rose-600 text-white hover:bg-rose-700">Xoá banner</button>
            </div>
          </div>
        </Transition>
      </div>
...
const fallbackImage = 'https://images.unsplash.com/photo-1483985988355-763728e1935b?w=600&h=300&fit=crop'

// Dữ liệu mẫu, đóng vai trò "database" giả lập
const banners = ref([
  {
    id: 1001,
    title: 'Banner Khuyến Mãi Mùa Hè 2024',
    ...
    description: 'Chương trình khuyến mãi hè rực rỡ với các ưu đãi giảm giá lên tới 50% cho tất cả các danh mục sản phẩm thời trang.',
  },
  {
    id: 1002,
    title: 'Banner Bộ Sưu Tập Thu Đông',
    ...
    description: 'Giới thiệu các mẫu thiết kế mới nhất nằm trong bộ sưu tập thời trang cao cấp mùa Thu Đông năm nay.',
  },
])

const tab = ref('all') // 'all' | 'recent'

// computed: đếm số bài Active / Draft, tự cập nhật khi banners thay đổi
const activeCount = computed(() => banners.value.filter(b => b.status === 'Active').length)
const draftCount = computed(() => banners.value.filter(b => b.status === 'Draft').length)

// computed: danh sách hiển thị theo tab đang chọn
const filteredBanners = computed(() => {
  if (tab.value === 'all') return banners.value
  return [...banners.value].sort((a, b) => b.id - a.id).slice(0, 2)
})
...
const selected = ref(null)   // banner đang xem chi tiết
...
function submitForm() {
  if (isCreating.value) {
    const newId = Math.max(...banners.value.map(b => b.id)) + 1
    ...
    if (idx !== -1) banners.value[idx] = { ...form.value }
  }
  closeAll()
}
...
function confirmDelete() {
  banners.value = banners.value.filter(b => b.id !== toDelete.value.id)
